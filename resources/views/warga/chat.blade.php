...
<div x-data="wargaChat()" x-init="init()" class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow-md overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 bg-gradient-to-r from-emerald-600 to-teal-600 text-white flex items-center gap-3">
        ...
        <div class="flex flex-col" style="height: 60vh;">
            ...
            <form @submit.prevent="kirim()" class="border-t border-gray-200 p-3 bg-white flex items-end gap-2">
                <textarea x-model="isi" rows="1" placeholder="Tulis pesan..." required
                          class="flex-1 resize-none px-4 py-3 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all text-sm max-h-32"></textarea>
                <button type="submit" :disabled="mengirim || !isi.trim()"
                        class="bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-5 py-3 rounded-xl font-semibold shadow-lg shadow-emerald-600/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-1">
