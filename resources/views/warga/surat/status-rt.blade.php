...
@section('title', "Status Pengajuan Surat - RT $rt RW $rw")
...
<div class="space-y-6 max-w-3xl">
    ...
    <div>
        <a href="{{ route('warga.rt.surat.index', ['rt' => $rt, 'rw' => $rw]) }}"
           class="inline-flex items-center space-x-1 text-gray-500 hover:text-gray-700 text-sm font-medium mb-4">
            ...
            <span>Kembali ke Surat Online</span>
        </a>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
            ...
            @endphp
            <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider whitespace-nowrap {{ $statusClass }}">{{ $pengajuan->status }}</span>
        </div>
        ...
        @if ($pengajuan->status === 'menunggu_ttd_fisik')
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 flex space-x-3">
                ...
                <div>
                    ...
                    <a href="{{ route('warga.rt.surat.pdf', ['rt' => $rt, 'rw' => $rw, 'kode' => $pengajuan->kode_tracking]) }}"
                       class="inline-flex items-center space-x-2 bg-amber-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-amber-700 transition-all">
        ...
        @if (in_array($pengajuan->status, ['disetujui_kades', 'selesai']) && !$pengajuan->butuh_ttd_fisik)
            <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex space-x-3">
                ...
                <div>
                    ...
                    <a href="{{ route('warga.rt.surat.pdf', ['rt' => $rt, 'rw' => $rw, 'kode' => $pengajuan->kode_tracking]) }}"
                       class="inline-flex items-center space-x-2 bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-all">
                        <span class="material-symbols-outlined text-[18px]">download</span>
