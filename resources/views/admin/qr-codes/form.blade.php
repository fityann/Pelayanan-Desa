...
<div class="flex flex-col gap-lg max-w-2xl">
    <div>
        <h1 class="text-headline-md font-bold text-on-surface">
            {{ $rtQrCode ? "Edit Wilayah RT {$rtQrCode->rt} / RW {$rtQrCode->rw}" : 'Tambah Wilayah Baru' }}
        </h1>
        <p class="text-body-sm text-on-surface-variant">{{ $rtQrCode ? 'Perbarui informasi wilayah dan status QR' : 'Daftarkan wilayah RT/RW baru untuk dibuatkan QR Code' }}</p>
    </div>
    ...
    <form method="POST"
        ...
        <div class="space-y-lg">
            <div class="grid grid-cols-2 gap-lg">
                <div>
                    <label class="text-label-sm font-bold text-on-surface block mb-xs">RT <span class="text-error">*</span></label>
                    <input type="text" name="rt" value="{{ old('rt', $rtQrCode?->rt ?? request('rt')) }}" required maxlength="3" inputmode="numeric"
                ...
                <div>
                    <label class="text-label-sm font-bold text-on-surface block mb-xs">RW <span class="text-error">*</span></label>
                    <input type="text" name="rw" value="{{ old('rw', $rtQrCode?->rw ?? request('rw')) }}" required maxlength="3" inputmode="numeric"
                           class="w-full bg-surface-container rounded-xl px-lg py-3 text-body-md outline-none focus:ring-2 focus:ring-primary/20 border border-outline-variant">
                </div>
            </div>
            <div>
                <label class="text-label-sm font-bold text-on-surface block mb-xs">Nama Wilayah / Nama Ketua RT</label>
                <input type="text" name="nama_rt" value="{{ old('nama_rt', $rtQrCode?->nama_rt) }}" maxlength="100" placeholder="cth: RT 01 RW 02"
                       class="w-full bg-surface-container rounded-xl px-lg py-3 text-body-md outline-none focus:ring-2 focus:ring-primary/20 border border-outline-variant">
            </div>
            <div>
            ...
            @if ($rtQrCode && $rtQrCode->qr_code_path)
                <div class="bg-surface-container rounded-xl p-md flex items-center gap-md">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-body-sm text-on-surface-variant">Digenerate {{ $rtQrCode->tanggal_generate?->translatedFormat('d M Y H:i') }}. Jika RT/RW diubah, QR lama akan dihapus dan harus digenerate ulang.</p>
                    </div>
                </div>
            @endif
        </div>
