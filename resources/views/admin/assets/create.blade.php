...
@section('title', 'Tambah Aset - SILAPU')
...
<div class="flex flex-col gap-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Tambah Aset</h1>
            <p class="text-sm text-slate-500">Input data inventaris atau aset baru</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.assets.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="asset_category_id" required class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        ...
                        @foreach($kategoriAsets as $kat)
                            <option value="{{ $kat->id }}" {{ old('asset_category_id') == $kat->id ? 'selected' : '' }}>{{ $kat->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="location" value="{{ old('location') }}" class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="condition" required class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        ...
                        @foreach(\App\Enums\KondisiAset::cases() as $kondisi)
                            <option value="{{ $kondisi->value }}" {{ old('condition') == $kondisi->value ? 'selected' : '' }}>{{ $kondisi->label() }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="acquisition_year" value="{{ old('acquisition_year') }}" min="1900" max="{{ date('Y') + 1 }}" class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Sumber Perolehan</label>
                    <input type="text" name="acquisition_source" value="{{ old('acquisition_source') }}" placeholder="Contoh: Dana Desa, Hibah..." class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Nilai Aset (Rp)</label>
                    <input type="number" step="0.01" name="value" value="{{ old('value') }}" class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="status" required class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        @foreach(\App\Enums\StatusAset::cases() as $status)
                            <option value="{{ $status->value }}" {{ old('status', 'aktif') == $status->value ? 'selected' : '' }}>{{ $status->label() }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" class="w-full bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">{{ old('description') }}</textarea>
                </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-5 py-2.5 rounded-lg font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-colors">Simpan Aset</button>
            </div>
        </form>
    </div>
</div>
