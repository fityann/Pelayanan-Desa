            ...
            <div class="grid grid-cols-2 gap-md">
                ...
                <div>
                    <select name="rw" class="w-full bg-surface-container rounded-xl px-lg py-3 text-body-md outline-none focus:ring-2 focus:ring-primary/20 border border-outline-variant">
                        ...
                        @for ($i = 1; $i <= 4; $i++)
                            <option value="{{ str_pad((string) $i, 2, '0', STR_PAD_LEFT) }}" {{ old('rw', $informasi->rw) == str_pad((string) $i, 2, '0', STR_PAD_LEFT) ? 'selected' : '' }}>RW {{ str_pad((string) $i, 2, '0', STR_PAD_LEFT) }}</option>
                        @endfor
                    </select>
