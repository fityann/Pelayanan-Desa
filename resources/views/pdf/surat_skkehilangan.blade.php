...
<head>
    <meta charset="utf-8">
    <title>Surat Keterangan Kehilangan - SILAPU Desa Puspamukti</title>
    <style>
...
<body>
    ...
    <div class="perihal-container">
        <div class="perihal-title">SURAT KETERANGAN KEHILANGAN</div>
        <div class="perihal-nomor">Nomor : 470/ {{ $surat->nomor_surat ?? $surat->kode_tracking_val }} / Des</div>
    </div>
    ...
    <div class="body-text">
        ...
        <table class="data-table">
            ...
            <tr><td class="label">Tempat/Tanggal Lahir</td><td class="colon">:</td><td>{{ $surat->user?->penduduk?->tempat_lahir ?? 'Tasikmalaya' }}, {{ $surat->user?->penduduk?->tanggal_lahir?->format('d-m-Y') ?? '-' }}</td></tr>
            <tr><td class="label">Jenis Kelamin</td><td class="colon">:</td><td>{{ $surat->user?->penduduk?->jenis_kelamin ?? '-' }}</td></tr>
            <tr><td class="label">Status Perkawinan</td><td class="colon">:</td><td>{{ $surat->user?->penduduk?->status_perkawinan ?? '-' }}</td></tr>
            <tr><td class="label">Pekerjaan</td><td class="colon">:</td><td>{{ $surat->user?->penduduk?->pekerjaan ?? '-' }}</td></tr>
            <tr><td class="label">Agama</td><td class="colon">:</td><td>{{ $surat->user?->penduduk?->agama ?? 'Islam' }}</td></tr>
            <tr><td class="label">Alamat</td><td class="colon">:</td><td>Kp. {{ $rawAlamat }} RT/RW {{ $rtNumber }}/{{ $rwNumber }} Desa Puspamukti</td></tr>
            <tr><td></td><td></td><td>Kecamatan Cigalontang Kabupaten Tasikmalaya</td></tr>
        </table>

        <p style="text-indent: 30px;">Benar yang Bernama tersebut diatas adalah penduduk Desa Puspamukti Kecamatan Cigalontang Kabupaten Tasikmalaya, dan menurut pengamatan kami benar yang bersangkutan telah kehilangan <strong>{{ $data['barang_hilang'] ?? $surat->keterangan ?? '......................................................................' }}</strong></p>

        <p style="text-indent: 30px;">Demikian surat kehilangan ini kami buat, untuk dapat dipergunakan seperlunya.</p>
    </div>
