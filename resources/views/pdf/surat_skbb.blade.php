...
<head>
    <meta charset="utf-8">
    <title>Surat Keterangan Belum Bekerja - SILAPU Desa Puspamukti</title>
    <style>
...
<body>
    ...
    <div class="kop-container">
        ...
        <div class="alamat">Jln. Desa Puspamukti No. 014 Desa Puspamukti Kec. Cigalontang 46463</div>
    </div>

    <div class="perihal-container">
        <div class="perihal-title">SURAT KETERANGAN BELUM BEKERJA</div>
        <div class="perihal-nomor">Nomor : 470/ {{ $surat->nomor_surat ?? $surat->kode_tracking_val }} / Des</div>
    </div>

    <div class="body-text">
        <p style="text-indent: 30px;">Yang bertanda tangan dibawah ini menerangkan bahwa :</p>

        <table class="data-table">
            <tr><td class="label">Nama</td><td class="colon">:</td><td>{{ $surat->pemohon_name ?? $surat->user?->name ?? 'Warga' }}</td></tr>
            <tr><td class="label">Tempat/Tgl Lahir</td><td class="colon">:</td><td>{{ $surat->user?->penduduk?->tempat_lahir ?? 'Tasikmalaya' }}, {{ $surat->user?->penduduk?->tanggal_lahir?->format('d-m-Y') ?? '-' }}</td></tr>
        ...
        <p>Orang tersebut diatas adalah benar penduduk Desa Puspamukti Kecamatan Cigalontang Kabupaten Tasikmalaya dan Domisili sebagai tercantum diatas dan Sepengetahuan kami bahwa orang tersebut <strong>Tidak Bekerja/Belum mempunyai penghasilan</strong>.</p>
